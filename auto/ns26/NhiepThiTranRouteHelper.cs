using ns10;
using ns42;

namespace ns26;

internal class NhiepThiTranRouteHelper
{
	private static uint[,] uint_0;

	private static uint[,] uint_1;

	private static uint[,] uint_2;

	private static uint[,] uint_3;

	private static uint[,] uint_4;

	private static GStruct23[] gstruct23_0;

	private static uint[,] uint_5;

	private static uint[,] uint_6;

	private static uint[,] uint_7;

	private static uint[,] uint_8;

	private static GStruct23[] gstruct23_1;

	private static uint[,] uint_9;

	private static uint[,] uint_10;

	private static uint[,] uint_11;

	private static uint[,] uint_12;

	private static GStruct23[] gstruct23_2;

	private static uint[,] uint_13;

	private static uint[,] uint_14;

	private static uint[,] uint_15;

	private static uint[,] uint_16;

	private static GStruct23[] gstruct23_3;

	private static uint[,] uint_17;

	private static uint[,] uint_18;

	private static uint[,] uint_19;

	private static uint[,] uint_20;

	private static GStruct23[] gstruct23_4;

	private static uint[,] uint_21;

	private static uint[,] uint_22;

	private static uint[,] uint_23;

	private static uint[,] uint_24;

	private static uint[,] uint_25;

	private static uint[,] uint_26;

	private static GStruct23[] gstruct23_5;

	private static uint[,] uint_27;

	private static uint[,] uint_28;

	private static uint[,] uint_29;

	private static GStruct23[] gstruct23_6;

	static NhiepThiTranRouteHelper()
	{
		uint[,] array_ = new uint[20, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_, 54488);
		uint_0 = array_;
		uint[,] array_2 = new uint[13, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_2, 54658);
		uint_1 = array_2;
		uint[,] array_3 = new uint[14, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_3, 54771);
		uint_2 = array_3;
		uint[,] array_4 = new uint[20, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_4, 54892);
		uint_3 = array_4;
		uint[,] array_5 = new uint[7, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_5, 55062);
		uint_4 = array_5;
		gstruct23_0 = new GStruct23[5]
		{
			new GStruct23
			{
				uint_0 = uint_0
			},
			new GStruct23
			{
				uint_0 = uint_1
			},
			new GStruct23
			{
				uint_0 = uint_2
			},
			new GStruct23
			{
				uint_0 = uint_3
			},
			new GStruct23
			{
				uint_0 = uint_4
			}
		};
		uint[,] array_6 = new uint[21, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_6, 55151);
		uint_5 = array_6;
		uint[,] array_7 = new uint[20, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_7, 55329);
		uint_6 = array_7;
		uint[,] array_8 = new uint[25, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_8, 55499);
		uint_7 = array_8;
		uint[,] array_9 = new uint[16, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_9, 55709);
		uint_8 = array_9;
		gstruct23_1 = new GStruct23[4]
		{
			new GStruct23
			{
				uint_0 = uint_5
			},
			new GStruct23
			{
				uint_0 = uint_6
			},
			new GStruct23
			{
				uint_0 = uint_7
			},
			new GStruct23
			{
				uint_0 = uint_8
			}
		};
		uint[,] array_10 = new uint[6, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_10, 55867);
		uint_9 = array_10;
		uint[,] array_11 = new uint[8, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_11, 55924);
		uint_10 = array_11;
		uint[,] array_12 = new uint[14, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_12, 55997);
		uint_11 = array_12;
		uint[,] array_13 = new uint[15, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_13, 56118);
		uint_12 = array_13;
		gstruct23_2 = new GStruct23[4]
		{
			new GStruct23
			{
				uint_0 = uint_9
			},
			new GStruct23
			{
				uint_0 = uint_10
			},
			new GStruct23
			{
				uint_0 = uint_11
			},
			new GStruct23
			{
				uint_0 = uint_12
			}
		};
		uint[,] array_14 = new uint[22, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_14, 56267);
		uint_13 = array_14;
		uint[,] array_15 = new uint[31, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_15, 56453);
		uint_14 = array_15;
		uint[,] array_16 = new uint[25, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_16, 56711);
		uint_15 = array_16;
		uint[,] array_17 = new uint[13, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_17, 56921);
		uint_16 = array_17;
		gstruct23_3 = new GStruct23[4]
		{
			new GStruct23
			{
				uint_0 = uint_13
			},
			new GStruct23
			{
				uint_0 = uint_14
			},
			new GStruct23
			{
				uint_0 = uint_15
			},
			new GStruct23
			{
				uint_0 = uint_16
			}
		};
		uint[,] array_18 = new uint[16, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_18, 57054);
		uint_17 = array_18;
		uint[,] array_19 = new uint[13, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_19, 57192);
		uint_18 = array_19;
		uint[,] array_20 = new uint[7, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_20, 57305);
		uint_19 = array_20;
		uint[,] array_21 = new uint[10, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_21, 57370);
		uint_20 = array_21;
		gstruct23_4 = new GStruct23[4]
		{
			new GStruct23
			{
				uint_0 = uint_17
			},
			new GStruct23
			{
				uint_0 = uint_18
			},
			new GStruct23
			{
				uint_0 = uint_19
			},
			new GStruct23
			{
				uint_0 = uint_20
			}
		};
		uint[,] array_22 = new uint[16, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_22, 57479);
		uint_21 = array_22;
		uint[,] array_23 = new uint[6, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_23, 57617);
		uint_22 = array_23;
		uint[,] array_24 = new uint[11, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_24, 57674);
		uint_23 = array_24;
		uint[,] array_25 = new uint[6, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_25, 57771);
		uint_24 = array_25;
		uint[,] array_26 = new uint[11, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_26, 57828);
		uint_25 = array_26;
		uint[,] array_27 = new uint[16, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_27, 57925);
		uint_26 = array_27;
		gstruct23_5 = new GStruct23[6]
		{
			new GStruct23
			{
				uint_0 = uint_21
			},
			new GStruct23
			{
				uint_0 = uint_22
			},
			new GStruct23
			{
				uint_0 = uint_23
			},
			new GStruct23
			{
				uint_0 = uint_24
			},
			new GStruct23
			{
				uint_0 = uint_25
			},
			new GStruct23
			{
				uint_0 = uint_26
			}
		};
		uint[,] array_28 = new uint[54, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_28, 58091);
		uint_27 = array_28;
		uint[,] array_29 = new uint[37, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_29, 58533);
		uint_28 = array_29;
		uint[,] array_30 = new uint[27, 2];
		EmbeddedResourceDataReader.CopyLengthPrefixedBlock(array_30, 58839);
		uint_29 = array_30;
		gstruct23_6 = new GStruct23[3]
		{
			new GStruct23
			{
				uint_0 = uint_27
			},
			new GStruct23
			{
				uint_0 = uint_28
			},
			new GStruct23
			{
				uint_0 = uint_29
			}
		};
	}

	public static uint[,] GetRouteToNhiepThiTran(int int_0, uint[] uint_30, uint[] uint_31)
	{
		if (uint_31 == null)
		{
			return null;
		}
		uint[,] result = null;
		switch (int_0)
		{
		case 37:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_3, uint_30, uint_31);
			break;
		case 11:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_1, uint_30, uint_31);
			break;
		case 1:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_2, uint_30, uint_31);
			break;
		case 80:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_0, uint_30, uint_31);
			break;
		case 78:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_5, uint_30, uint_31);
			break;
		case 176:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_6, uint_30, uint_31);
			break;
		case 162:
			result = MapTravelDataHelper.SelectBestRouteCoordinates(gstruct23_4, uint_30, uint_31);
			break;
		}
		return result;
	}
}
