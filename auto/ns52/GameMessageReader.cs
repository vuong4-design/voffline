...
internal class GameMessageReader
{
	...
	{
		uint messageRoot = WindowsInteropHelper.ReadProcessUInt32(GameConfigurationManager.memorySignatureScanConfig_165.resolvedValue, accountConfig.int_137);
		uint messageContainer = WindowsInteropHelper.ReadProcessUInt32(messageRoot + GameConfigurationManager.memorySignatureScanConfig_168.resolvedValue, accountConfig.int_137);
	...
	public static string ReadMessages(CharacterAccountConfig accountConfig, bool includeMessageId = false)
	{
		...
		uint messageRoot = WindowsInteropHelper.ReadProcessUInt32(GameConfigurationManager.memorySignatureScanConfig_165.resolvedValue, accountConfig.int_137);
		uint messageList = WindowsInteropHelper.ReadProcessUInt32(messageRoot + GameConfigurationManager.memorySignatureScanConfig_167.resolvedValue, accountConfig.int_137);
		if (messageList == 0)
		{
			return messages;
		}
		for (uint messageIndex = 0u; messageIndex < 10; messageIndex++)
		{
			...
			if (includeMessageId)
			{
				int messageId = (int)WindowsInteropHelper.ReadProcessUInt32(messageAddress + GameConfigurationManager.memorySignatureScanConfig_170.resolvedValue, accountConfig.int_137);
				if (messageId <= 0)
				{
					continue;
				}
