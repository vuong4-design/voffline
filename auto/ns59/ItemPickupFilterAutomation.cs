...
internal class ItemPickupFilterAutomation
{
	...
	private static void RunForCharacter(int int_2)
	{
		...
		while (true)
		{
			...
			if (!characterAccountConfig.bool_7 && !characterAccountConfig.bool_8 && CommonUtility.GetElapsedMilliseconds(num3) >= 3000L)
			{
				try
				{
	...
	public static bool ApplyInventoryPickupFilterRules(CharacterAccountConfig characterAccountConfig_0)
	{
		...
		for (uint num6 = 1u; num6 < GameConfigurationManager.int_1 && num4 > num5; num6++)
		{
			...
			string text = GameTextEncodingHelper.DecodeNullTerminatedUtf7(array).Trim();
